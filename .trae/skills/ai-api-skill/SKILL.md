...
- **平台名称**：
- **官网地址**：https://gappzz.com

## 认证方式

> **API Key 已配置**：`sk-30c12938d0030f142f0ad5e1712f63ef936652f947897d75`

所有接口使用 Bearer Token 认证：
```
Authorization: Bearer sk-30c12938d0030f142f0ad5e1712f63ef936652f947897d75
```

API Key 获取方式（按优先级）：
1. 读取项目根目录 `.env` 文件中的 `API_KEY` 变量
2. 如果 `.env` 中没有，读取本 SKILL.md 中记录的 Key（已配置）
3. 如果都找不到，提示用户提供 API Key，并引导用户前往  官网（https://gappzz.com）登录后在「API 密钥管理」中创建
...
```
GET https://api.lk888.ai/api/v1/skills
```
...
```
GET https://api.lk888.ai/api/v1/skills/guide
```
...
2. **调用付费接口前**，先通过 `GET https://api.lk888.ai/api/v1/skills/balance` 检查余额
3. **余额不足时**，提示用户前往  官网（https://gappzz.com）充值算力
4. **API Key 错误时**，提示用户前往  官网（https://gappzz.com）登录并在「API 密钥管理」中创建或检查密钥
...
```
POST https://api.lk888.ai/api/v1/skills/feedback
...
```
GET https://api.lk888.ai/api/v1/skills/feedback?id={feedback_id}
```
